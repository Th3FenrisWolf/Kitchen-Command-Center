...
[assembly: UIPage(
    parentType: typeof(VariantReviewEditSection),
    slug: "edit",
    uiPageType: typeof(VariantReviewEditPage),
    name: "Edit review",
...
/// </remarks>
public sealed class VariantReviewEditPage(
...
{
    [PageParameter(typeof(IntPageModelBinder), typeof(VariantReviewEditSection))]
    ...
    /// <remarks>
    /// The variant and the review are bound from independent URL segments and the platform relates
    /// them in no way, so without this a review left against another variant opens under this
    ...
    public override async Task<PageValidationResult> ValidatePage()
    {
        ...
        return new PageValidationResult
        {
            IsValid = identity.Is(ContributionContentTypes.RecipeVariant)
                && review is not null
                && review.VariantGuid == identity.ContentItemGuid,
            ErrorMessageKey = ContributionTabMessages.ReviewNotOnThisVariant,
        };
    }
        ...
        EditTemplateClientProperties properties)
    {
        ...
        properties.Headline = "Edit review";
