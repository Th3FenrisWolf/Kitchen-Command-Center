...
<template>
  <section class="mx-auto grid w-full max-w-xl gap-5 px-4 py-10">
    <a :href="backUrl" class="text-onyx-light">&larr; <ResourceString for="BackToProfile" /></a>
    <h1 class="text-3xl"><ResourceString for="AccountSettings" /></h1>

    <!-- Profile card -->
    <form class="grid gap-4 rounded-3xl bg-bone p-6 shadow-primary" @submit.prevent="saveProfile">
      <h2 class="text-xl"><ResourceString for="Profile" /></h2>

      <label class="grid gap-2">
        <span class="text-sm text-onyx-light"><ResourceString for="FirstName" /></span>
        <InputField required type="text" v-model="firstName" name="FirstName" autocomplete="given-name" />
      </label>
      <label class="grid gap-2">
        <span class="text-sm text-onyx-light"><ResourceString for="LastName" /></span>
        <InputField required type="text" v-model="lastName" name="LastName" autocomplete="family-name" />
      </label>
      ...
      <button
        ...
        type="submit"
        class="justify-self-end rounded-2xl bg-surface-500 px-4 py-2 text-bone disabled:opacity-50"
      >
    ...
    <!-- Password card -->
    <form class="grid gap-4 rounded-3xl bg-bone p-6 shadow-primary" @submit.prevent="changePassword">
    ...
    <a :href="logoutUrl" class="justify-self-center rounded-2xl border border-maroon px-4 py-2 text-maroon">
