...
public class PageBuilderMountPreloadTagHelperTests
{
    ...
    public async Task ProcessAsync_PageBuilderEdit_EmitsModulepreloadLink()
    {
        ...
        _ = await Assert.That(output.TagName).IsEqualTo("link");
        _ = await Assert.That(output.TagMode).IsEqualTo(TagMode.SelfClosing);
        _ = await Assert.That(output.Attributes["rel"].Value).IsEqualTo("modulepreload");
        _ = await Assert.That(output.Attributes["href"].Value).IsEqualTo("/assets/pageBuilderMount-abc123.js");
    }
    ...
    public async Task ProcessAsync_DevServerEnabled_EmitsDevServerHref()
    {
        ...
        _ = await Assert.That(output.Attributes["href"].Value).IsEqualTo("http://localhost:5173/Features/PageBuilderMount.ts");
    }
