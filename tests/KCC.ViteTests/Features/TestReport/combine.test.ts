...
describe('buildReport', () => {
        ...
        groups: [
          {
            className: 'C',
            summary: { total: 1, passed: 1 },
            tests: [{ displayName: 't', status: 'passed', durationMs: 1 }],
          },
        ],
      }),
    },
        ...
        testResults: [
          {
            ...
            startTime: 0,
            endTime: 2,
            assertionResults: [{ ancestorTitles: ['G'], title: 'ok', status: 'passed', duration: 2, failureMessages: [] }],
          },
        ],
      }),
    },
    {
      descriptor: { id: 'admin-frontend', label: 'Admin Frontend', type: 'vitest' },
      kind: 'missing',
  ...
  it('captures a bad source as an error suite instead of throwing', () => {
    const { report } = buildReport(
      [{ descriptor: { id: 'unit', label: 'Unit', type: 'dotnet' }, kind: 'tunit-html', content: '<html>no data</html>' }],
      {},
    )
